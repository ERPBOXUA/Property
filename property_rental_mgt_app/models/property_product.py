...
class ProductProduct(models.Model):
    ...
    state = fields.Selection([('draft','Draft'),('rent','Rentable'),('sale','Saleable'),('reserve','Reserve'),('sold','Sold'),('cancel','Cancelled')], string="Property Status", help='State of the Property')
    ...
    image_ids = fields.Many2many('property.image','property_image_default_rel','property_id','image_id', string="Images")
    ...
    def auto_deduct_fine_on_delay_payment(self):
        invoices = self.env['account.move'].search([('property_id','!=',False),('invoice_date_due','!=',False),('payment_state','=','not_paid')])
        if invoices:
            ...
            if invoice.property_id.allow_delay_fine:
                ...
                if after2_date == fields.Date.today():
                    ...
                    values = {
                        ...
                        'invoice_line_ids': [(0,0,{'name':invoice.property_id.name+'-'+'Delay Fine Payment', 'product_id':invoice.property_id.id, 'account_id': income_account, 'price_unit': fine_amount})],
                        }
